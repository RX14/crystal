...
- **(breaking-change)** The `DivisionByZero` exception was renamed `DivisionByZeroError`. ([#5395], thanks @sdogruyol)
- A bootstrap windows port has been added to the standard library. It's not usable for real programs yet. ([#5339], [#5484] and [#5448], thanks @RX14)
- `Number#round` can now be called without specifying the number of digits to round to: it rounds to the nearest whole number. ([#5397])
- The compiler can now take pointers of external C library global variables. ([#4845], thanks @larubujo)

## Standard library bugs fixed
- `String#sub` now correctly handles negative indexes. ([#5491], thanks @MakeNowJust)
...
- The junit spec formatter was fixed to emit the correct XML. ([#5463], thanks @hanneskaeufler)
...
- Fixed parsing some regexes in default arguments. ([#5481], thanks @MakeNowJust)
- Static libraries are now found correctly when using the `--static` compiler flag. ([#5385], thanks @jreinert)
- Error messages for unterminated literals were improved. ([#5409], thanks @straight-shoota)
- The `README.md` generated by `crystal tool init` was improved to put git commands in backticks. ([#5475], thanks @woodruffw)
...
## Misc
- The platform-specific parts of `File` and `IO::FileDescriptor` were moved to `Crystal::System`, as part of preparation for the windows port. ([#5333], thanks @RX14)
- The platform-specific parts of `Dir` were moved to `Crystal::System`, as part of preparation for the windows port. ([#5447], thanks @RX14)
- The build on OpenBSD was fixed. ([#5387], thanks @wmoxam)
- All of the scripts and makefiles in the repo now depend on `sh` instead of `bash` for greater portability. ([#5468], thanks @j8r)
- Several documentation fixes. ([#5425], thanks @MakeNowJust)
- Several miscellaneous minor code cleanups. ([#5499], [#5502] and [#5507], thanks @chastell and @lachlan)
